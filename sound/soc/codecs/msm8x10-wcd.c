...
//Kevin Shiu QCOM patch adjust Tx analog gain via UCM file
#if 0
...
struct msm8x10_wcd_priv {
	...
	struct delayed_work hs_detect_work; //Kevin Shiu Fix: headset can't be detected after upgrade 1.3.4402.1
			     ...
			     unsigned int value)
{
	...
	/* Kevin Shiu Avoid changing SPK's REG after ESD test */
...
static const struct snd_kcontrol_new msm8x10_wcd_snd_controls[] = {
	...
	SOC_SINGLE_S8_TLV("IIR1 INP4 Volume",
			  ...
			  -84,	40, digital_gain),
//Kevin Shiu QCOM patch adjust mic analog gain via UCM file
...
//Kevin Shiu Fix: headset can't be detected after upgrade 1.3.4402.1
		    ...
		    struct wcd9xxx_mbhc_config *mbhc_cfg)
{
	...
	//Kevin Shiu 20140410 Fix: headset can't be detected after upgrade 1.3.4402.1
	#if 1
...
static int msm8x10_wcd_codec_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&msm8x10_wcd_priv->hs_detect_work,
			delayed_hs_detect_fn); //Kevin Shiu 20140410 Fix: headset can't be detected after upgrade 1.3.4402.1
